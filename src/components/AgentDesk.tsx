...
function DeskScene({ agent }: { agent: Agent }) {
  ...
  return (
    <svg viewBox="0 0 200 120" className="w-full h-auto">
      {/* Desk surface */}
      <rect x="20" y="88" width="160" height="6" rx="2" fill="hsl(217, 33%, 17%)" />
      {/* Monitor stand */}
      <rect x="88" y="80" width="24" height="8" rx="2" fill="hsl(217, 33%, 17%)" />

      {/* Main monitor */}
      <rect x="40" y="20" width="120" height="60" rx="4" fill="hsl(220, 30%, 8%)" stroke="hsl(217, 33%, 17%)" strokeWidth="1.5" />
      ...
      {hasDualMonitor && (
        <>
          <rect x="170" y="30" width="20" height="30" rx="2" fill="hsl(220, 30%, 8%)" stroke="hsl(217, 33%, 17%)" strokeWidth="1" />
          <rect x="172" y="32" width="16" height="26" rx="1" fill={cfg.screenFill} opacity={cfg.screenOpacity * 0.6} />
        </>
      )}

      {/* Keyboard */}
      <rect x="60" y="96" width="80" height="10" rx="2" fill="hsl(217, 33%, 17%)" stroke="hsl(217, 33%, 22%)" strokeWidth="0.5" />
      {/* Typing indicator */}
      {agent.status === 'active' && (
        <rect x="85" y="99" width="4" height="4" rx="1" fill="hsl(217, 91%, 60%)" opacity="0.5">
          <animate attributeName="opacity" values="0.5;1;0.5" dur="0.3s" repeatCount="indefinite" />
        </rect>
      )}

      {/* Coffee mug */}
      <ellipse cx="175" cy="94" rx="8" ry="3" fill="hsl(217, 33%, 17%)" />
      <rect x="167" y="94" width="16" height="12" rx="2" fill="hsl(217, 33%, 17%)" />
      {/* Steam */}
      {agent.status === 'active' && (
        <>
          <path d="M172,90 C172,86 174,84 174,80" fill="none" stroke="hsl(215, 16%, 65%)" strokeWidth="0.5" opacity="0.3">
            <animate attributeName="opacity" values="0.1;0.4;0.1" dur="2s" repeatCount="indefinite" />
          </path>
          <path d="M176,91 C176,87 178,85 178,81" fill="none" stroke="hsl(215, 16%, 65%)" strokeWidth="0.5" opacity="0.2">
      ...
      {hasPlant && (
        <>
          <rect x="22" y="80" width="10" height="8" rx="1" fill="hsl(217, 33%, 20%)" />
          <circle cx="27" cy="76" r="5" fill="hsl(142, 50%, 30%)" />
          <circle cx="24" cy="78" r="3" fill="hsl(142, 50%, 25%)" />
        </>
      )}
      {/* Sticky notes */}
      {hasStickyNotes && (
