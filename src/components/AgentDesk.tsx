...
const statusConfig = {
  active: { border: 'border-primary/30', stripe: 'bg-green', screenFill: 'hsl(199, 89%, 48%)', screenOpacity: 0.15, dot: 'bg-green dot-glow-green', screenClass: 'screen-active' },
  idle: { border: 'border-border', stripe: 'bg-muted-foreground', screenFill: 'hsl(214, 18%, 89%)', screenOpacity: 0.3, dot: 'bg-muted-foreground', screenClass: '' },
  error: { border: 'border-red/30', stripe: 'bg-red', screenFill: 'hsl(0, 72%, 51%)', screenOpacity: 0.12, dot: 'bg-red dot-glow-red', screenClass: 'screen-error' },
  offline: { border: 'border-border/50', stripe: 'bg-border', screenFill: 'hsl(210, 16%, 95%)', screenOpacity: 0.3, dot: 'bg-border', screenClass: '' },
};
...
function DeskScene({ agent }: { agent: Agent }) {
  ...
  return (
    <svg viewBox="0 0 200 120" className="w-full h-auto">
      ...
      {agent.status === 'active' && (
        <>
          <rect x="50" y="30" width="40" height="2" rx="1" fill="hsl(215, 25%, 15%)" opacity="0.15" />
          <rect x="50" y="36" width="60" height="2" rx="1" fill="hsl(215, 25%, 15%)" opacity="0.1" />
          <rect x="50" y="42" width="35" height="2" rx="1" fill="hsl(215, 25%, 15%)" opacity="0.15" />
          <rect x="50" y="48" width="55" height="2" rx="1" fill="hsl(215, 25%, 15%)" opacity="0.1" />
          <rect x="50" y="54" width="45" height="2" rx="1" fill="hsl(215, 25%, 15%)" opacity="0.15" />
          <rect x="50" y="60" width="50" height="2" rx="1" fill="hsl(215, 25%, 15%)" opacity="0.1" />
          {/* Cursor blink */}
          <rect x="110" y="48" width="6" height="2" rx="1" fill="hsl(199, 89%, 48%)" opacity="0.8">
      ...
      {(agent.currentState === 'Deploying' || agent.currentState === 'Testing') && (
        <>
          <rect x="60" y="48" width="80" height="4" rx="2" fill="hsl(214, 18%, 89%)" />
          <rect x="60" y="48" width="52" height="4" rx="2" fill="hsl(199, 89%, 48%)">
      ...
      {hasStickyNotes && (
        <>
          <rect x="10" y="24" width="14" height="12" rx="1" fill="hsl(43, 96%, 56%)" opacity="0.4" transform="rotate(-5 17 30)" />
          <rect x="12" y="40" width="14" height="12" rx="1" fill="hsl(199, 89%, 48%)" opacity="0.25" transform="rotate(3 19 46)" />
        </>
      )}
      {/* Figurines */}
      {hasFigurines && (
